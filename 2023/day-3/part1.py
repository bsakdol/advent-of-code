...
def is_symbol(char: str) -> bool:
    ...
    """
    return char.strip() != "" and char not in NOT_SYMBOLS
...
def get_numbers_adjacent_to_symbols (grid: list[list[str]], x: int, y: int) -> list[int]:
    """
    ...
    Returns:
        list[int]: A list of numbers discovered adjacent to symbols.
    """
    all_numbers = []
...
def sum_of_part_numbers(grid: list[list[str]]) -> int:
    ...
    for y, row in enumerate(grid):
        for x, cell in enumerate(row):
            if is_symbol(cell):
                all_numbers.extend(get_numbers_adjacent_to_symbols(grid, x, y))
